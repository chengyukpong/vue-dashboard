...
export function addCustomer(input: CreateCustomerInput): User {
  const customer: User = {
    id: Math.max(0, ...customers.map(c => c.id)) + 1,
    ...input,
    avatar: {
      src: `https://i.pravatar.cc/128?u=${Math.max(0, ...customers.map(c => c.id)) + 1}`
    },
    status: 'subscribed',
    location: 'New York, USA'
  }
...
export function removeCustomers(ids: number[]): void {
  const idSet = new Set(ids)
  for (let i = customers.length - 1; i >= 0; i--) {
    if (idSet.has(customers[i]!.id)) {
